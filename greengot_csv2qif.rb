...
Qif::Writer.open(output_file, 'Bank', 'dd/mm/yyyy') do |qif|
  transactions.each_with_index do |transaction, index|
    warn "Processing #{index + 1}/#{transactions.size}"

    unless transaction['amount']['currency'] == 'EUR'
      raise "Payment #{transaction['id']} is not in euros but in #{transaction['amount']['currency']}"
    end
